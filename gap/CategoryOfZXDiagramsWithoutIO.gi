...
InstallMethod( ZXFusionRule,
  ...
  function ( ZX, type, c, l1, l2 )
    local e, labels_L, edges_L, L, labels_R, edges_R, R, labels_K, K, l, r;
    
    Assert( 0, c > 0 );
    
    e := l1 + l2;
    
    labels_L := Concatenation( ListWithIdenticalEntries( e, "neutral" ), [ type, type ], ListWithIdenticalEntries( c, "neutral" ) );
    
    edges_L := Concatenation(
                       List( [ 0 .. l1 - 1 ], i -> Pair( i, e ) ),
                       List( [ 0 .. l2 - 1 ], i -> Pair( l1 + i, e + 1 ) ),
                       Concatenation( List( [ 0 .. c - 1 ], i -> [ Pair( e + 2 + i, e ), Pair( e + 2 + i, e + 1 ) ] ) ) );
    
    L := ObjectConstructor( ZX, Pair( labels_L, edges_L ) );
    ...
    R := ObjectConstructor( ZX, Pair( labels_R, edges_R ) );
    
    labels_K := ListWithIdenticalEntries( e, "neutral" );
    
    K := ObjectConstructor( ZX, Pair( labels_K, [ ] ) );
    
    l := MorphismConstructor( ZX,
                 K,
                 Pair( [ 0 .. e - 1 ], [ ] ),
                 L );
